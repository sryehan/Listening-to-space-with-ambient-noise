...
A3=abs(d)
phase = atan((imag(d))./(real(d)))
t = linspace(1,5,100)

f=A3(2,5,6).*cos(3*t+phase(2,5,6))
plot(t,f)
keyboard
    ...
    function [J,DJ] = adjoint_state_gradient(dm)
        ...
        for ii = 1:nf
            ...
            for jj = 1:ns
                ...
                % kernel
                kk = real(2*pi*all_freqs(ii))^2*real(u(win_inds).*conj(q(win_inds)));
                K = zeros(1,100*100);
                K(win_inds) = K(win_inds) + kk';
                figure
                surf(1:100,1:100,reshape(K,100,100))
                view(2)
                Update the gradient
        ...
        JJ = zeros(1,100*100);
        JJ(win_inds) = JJ(win_inds) + DJ';
        surf(1:100,1:100,reshape(JJ,100,100))
        view(2)
        fprintf('\n')
    end
end