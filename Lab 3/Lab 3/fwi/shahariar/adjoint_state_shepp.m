...
maxit=20;						% maximum number of LBFGS iterations
...
% source_info.type='hline';		% create a horizontal line
% source_info.bounds=[20 80]/100;		
% source_info.height= 80/100;			
% sources=sources_and_receivers(ns,source_info);
% 
% source_info.type='vline';		% create a vertical line
% source_info.bounds=[20 80]/100;		
% source_info.pos= 80/100;			
% sources=sources_and_receivers(ns,source_info);

receiver_info.type='ring';		% create a ring of receivers
receiver_info.center=[.5 .5];	% center of ring of receivers
receiver_info.radius=.35;		% radius of ring of receivers
...
phan = phantom2(dom,.35,ctr,smoothness);
...
subplot(2,1,1)
if sum(m<0)>0
    warning('Negative values encountered in m!')
end
